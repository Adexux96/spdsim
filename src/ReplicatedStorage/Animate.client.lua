...
function onSwimming(speed)
	for _,track in pairs(Humanoid:GetPlayingAnimationTracks()) do 
		if track.Animation.Name == "fight_idle" then
			track:Stop()
		end
	end
	if speed > 1.00 then
...
local isSwimming = game.Players.LocalPlayer:WaitForChild("leaderstats"):WaitForChild("temp"):WaitForChild("isSwimming")

local function swimmingLoopCheck()
	while isSwimming.Value do 
		onSwimming(Character:WaitForChild("HumanoidRootPart").Velocity.Magnitude)
		task.wait(1/30)
	end
end
...
isSwimming:GetPropertyChangedSignal("Value"):Connect(function()
	if isSwimming.Value then
		local newThread = coroutine.wrap(swimmingLoopCheck)
		newThread()
	end
end)

if isSwimming.Value then
	--print("you're swimming right now")
	local newThread = coroutine.wrap(swimmingLoopCheck)
	newThread()
end
