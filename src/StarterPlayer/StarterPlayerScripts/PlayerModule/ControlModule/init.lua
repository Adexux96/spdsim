...
function ControlModule.new()
	...
	currentBinds["ControlScriptRenderstep"] = true
...
function ControlModule:Enable(enable: boolean?)
	...
	if enable then
		if not currentBinds["ControlScriptRenderstep"] then
			currentBinds["ControlScriptRenderstep"] = true
			RunService:BindToRenderStep("ControlScriptRenderstep", Enum.RenderPriority.Input.Value, function(dt)
				self:OnRenderStepped(dt)
			end)
...
function ControlModule:Disable()
	if self.activeController then
		if currentBinds["ControlScriptRenderstep"] then
			currentBinds["ControlScriptRenderstep"] = nil
...
function ControlModule:OnRenderStepped(dt)
	if isRolling.Value or _G.cutscenePlaying then return end
...
function ControlModule:UpdateTouchGuiVisibility()
	if _G.cutscenePlaying == true and self.touchGui then 
